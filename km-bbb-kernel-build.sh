...
if [ -f .config ] ; then
	...
	echo "${Purple}make ARCH=arm CROSS_COMPILE=arm-linux-gnueabihf- km-bbb-am335x_defconfig${NC}"
	make ARCH=arm CROSS_COMPILE=arm-linux-gnueabihf- km-bbb-am335x_defconfig
        if [ -f .config ] ; then
